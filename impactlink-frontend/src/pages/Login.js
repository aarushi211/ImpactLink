...
function Field({ label, children, hint }) {
  return (
    <div style={{ marginBottom: 18 }}>
      <label style={{ display: "block", color: "#888", fontSize: 11,
        fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em",
        marginBottom: 6 }}>
        {label}
        {hint && <span style={{ color: "#444", fontWeight: 400,
          textTransform: "none", letterSpacing: 0, marginLeft: 6 }}>({hint})</span>}
      </label>
      {children}
    </div>
  );
...
export default function Login() {
  ...
  const handleRegister = async (e) => {
    ...
    setError(""); setLoading(true);
    try {
      const profile = await register(regEmail, regPass, orgName);
      // Immediately enrich profile with step-2 data
      if (mission || location || causeArea) {
        const { updateProfile } = require("../context/AuthContext.js");
        // We'll do this via API directly since we just registered
  ...
  return (
    <div style={{
      ...
      >
        ...
        <h2 style={{ fontSize: 38, fontWeight: 800, color: "#fff",
